...
%token <node> ERROR ADD SUB MUL DIV LT LTE GT GTE EQ NEQ ASSIN SEMICOLON COMMA LPARENTHESE RPARENTHESE LBRACKET RBRACKET LBRACE RBRACE ELSE IF INT RETURN VOID WHILE ID NUMBER LETTER ARRAY EOL COMMENT BLANK
...
program:declaration_list {gt->root = newSyntaxTreeNode("program"); SyntaxTreeNode_AddChild(gt->root, $<node>1);}
        ;
...
var_declaration:type_specifier ID SEMICOLON {$<node>$ = newSyntaxTreeNode("var-declaration"); SyntaxTreeNode_AddChild($<node>$, $<node>1); SyntaxTreeNode_AddChild($<node>$, $<node>2);SyntaxTreeNode_AddChild($<node>$, $<node>3);}
		| type_specifier ID LBRACKET NUMBER RBRACKET SEMICOLON {$<node>$ = newSyntaxTreeNode("var-declaration"); SyntaxTreeNode_AddChild($<node>$, $<node>1); SyntaxTreeNode_AddChild($<node>$, $<node>2); SyntaxTreeNode_AddChild($<node>$, $<node>3); SyntaxTreeNode_AddChild($<node>$, $<node>4);SyntaxTreeNode_AddChild($<node>$, $<node>5);SyntaxTreeNode_AddChild($<node>$, $<node>6);}
		;
...
statement:expression_stmt {$<node>$ = newSyntaxTreeNode("statement");SyntaxTreeNode_AddChild($<node>$, $<node>1);}
        ...
        | iteration_stmt {$<node>$ = newSyntaxTreeNode("statement");SyntaxTreeNode_AddChild($<node>$, $<node>1);}
...
expression_stmt:expression SEMICOLON {$<node>$ = newSyntaxTreeNode("expression-stmt");SyntaxTreeNode_AddChild($<node>$, $<node>1);SyntaxTreeNode_AddChild($<node>$, $<node>2);}
		...
		;
selection_stmt:IF LPARENTHESE expression RPARENTHESE statement %prec LOWER_THAN_ELSE {$<node>$ = newSyntaxTreeNode("selection-stmt");SyntaxTreeNode_AddChild($<node>$, $<node>1);SyntaxTreeNode_AddChild($<node>$, $<node>2);SyntaxTreeNode_AddChild($<node>$, $<node>3);SyntaxTreeNode_AddChild($<node>$, $<node>4);SyntaxTreeNode_AddChild($<node>$, $<node>5);}
